      ...
      <div className="container">
        <div className="itinerary-grid">
          ...
          <div>
            ...
            <p style={{ fontFamily: 'var(--font-geist-mono)', fontSize: 12, color: 'var(--color-steel-gray)', lineHeight: 1.65, marginBottom: 20, maxWidth: 480 }}>
            ...
            <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 24 }}>
              {[SAMPLE_ITINERARY.destination, SAMPLE_ITINERARY.duration, SAMPLE_ITINERARY.style].map(m => (
                <span key={m} className="pill" style={{ pointerEvents: 'none', fontSize: 11 }}>{m}</span>
              ))}
            </div>
                ...
                >
                  <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '13px 16px' }}>
                    <div style={{
                      ...
                      <span style={{ fontFamily: 'var(--font-geist-mono)', fontSize: 11, fontWeight: 500, color: expandedDay === day.day ? 'var(--color-onyx-black)' : 'var(--color-steel-gray)' }}>
                        {String(day.day).padStart(2, '0')}
                      </span>
                    </div>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', gap: 8 }}>
                        <p style={{ fontFamily: 'var(--font-geist-mono)', fontWeight: 500, fontSize: 13, color: 'var(--color-pure-white)', lineHeight: 1.3 }}>{day.title}</p>
                        <span style={{ fontFamily: 'var(--font-geist-mono)', fontSize: 10, color: 'var(--color-steel-gray)', whiteSpace: 'nowrap', flexShrink: 0 }}>{day.location}</span>
                      </div>
                    </div>
                  ...
                  <div style={{ overflow: 'hidden', maxHeight: expandedDay === day.day ? 120 : 0, transition: 'max-height 0.35s var(--ease-out)' }}>
                    <p style={{ padding: '0 16px 14px 58px', fontFamily: 'var(--font-geist-mono)', fontSize: 12, color: 'var(--color-steel-gray)', lineHeight: 1.7 }}>{day.desc}</p>
                  </div>
                </div>
              ))}
              {/* Blur teaser */}
                  ...
                  {SAMPLE_ITINERARY.days.slice(6).map(day => (
                    <div key={day.day} style={{ background: 'var(--color-carbon)', border: '1px solid var(--color-zinc-hairline)', borderRadius: 'var(--radius-xl)', padding: '13px 16px', marginBottom: 6 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                        ...
                        <p style={{ fontFamily: 'var(--font-geist-mono)', fontWeight: 500, fontSize: 13, color: 'var(--color-ash-gray)' }}>{day.title}</p>
                      </div>
                    </div>
                  ))}
                </div>
          ...
          <div style={{ position: 'sticky', top: 72 }}>
            <div style={{
              ...
              <div style={{ height: 180, overflow: 'hidden', position: 'relative' }}>
                ...
                <span style={{ position: 'absolute', top: 12, left: 12, fontFamily: 'var(--font-geist-mono)', fontSize: 9, color: 'var(--color-onyx-black)', background: 'var(--color-highlighter-lime)', padding: '3px 8px', borderRadius: 'var(--radius-full)', fontWeight: 500 }}>
                  Pangong Tso
                </span>
              </div>
              <div style={{ padding: '20px' }}>
                ...
                <p style={{ fontFamily: 'var(--font-geist-mono)', fontSize: 12, color: 'var(--color-steel-gray)', lineHeight: 1.6, marginBottom: 16 }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 16 }}>
                  {['Custom vehicle routing & inner line permits', 'Stays in luxury camps, palaces, & villas', 'Dedicated 24/7 travel advisor on WhatsApp', 'Curated local guides & expert botanists'].map(item => (
                    <div key={item} style={{ display: 'flex', gap: 8, alignItems: 'flex-start' }}>
                      ...
                      <span style={{ fontFamily: 'var(--font-geist-mono)', fontSize: 11, color: 'var(--color-steel-gray)', lineHeight: 1.5 }}>{item}</span>
                    </div>
                  ))}
                </div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                  <button className="btn-primary" style={{ width: '100%', justifyContent: 'center', fontSize: 13 }} onClick={() => document.getElementById('planner')?.scrollIntoView({ behavior: 'smooth' })}>
                    Design My Plan — Free
                  </button>
                  <a
                    ...
                    className="btn-ghost"
                    style={{ textAlign: 'center', justifyContent: 'center', fontSize: 13 }}
                  >
                    WhatsApp with Specialist
                  </a>
                </div>
                <p style={{ fontFamily: 'var(--font-geist-mono)', fontSize: 10, color: 'var(--color-ash-gray)', textAlign: 'center', marginTop: 10 }}>No hidden costs · Complimentary consultation</p>
              </div>
            </div>
          </div>
        </div>
